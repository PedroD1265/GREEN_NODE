...
if (fs.existsSync(distPath)) {
  ...
  // SPA fallback: non-API/uploads routes serve index.html
  ...
  console.log('[Server] No dist/ found — frontend not served (run npm run build first)');
}
...
if (config.jwtSecret === 'green-node-demo-secret-change-in-production' && config.appMode !== 'demo') {
  console.warn('[SECURITY] Using default JWT secret! Set JWT_SECRET in environment variables.');
}

getProviders();

app.listen(config.port, '0.0.0.0', () => {
  console.log(`[API] Server running on http://0.0.0.0:${config.port} (mode: ${config.appMode})`);
});
